### tLDBPPdensity.R
### Fit a single atoms linear dependent Bernstein-Dirichlet model for
...
#' @param ngrid =100,
#' @param grid =NULL,
#' @param compute.band =FALSE,
#' @param type.band ="PD",
#' @param data =sys.frame(sys.parent()),
#' @param na.action =na.fail,
#' @param work.dir =NULL
#' 
         ...
         work.dir=NULL)
{
       ...
       #########################################################################################
         ...
         if(is.null(mcmc$slicetheta))
         {
			slicetheta <- 0.01
		 }
		 else
		 {
			slicetheta <- mcmc$slicetheta
		 }

		 slice  <- c(slicebeta,slicegp,slicetheta)
       ...
       #########################################################################################
		 ...
		 randsave <- matrix(0,nrow=nsave,ncol=p*(maxn-1)+maxn)
       ...
       #########################################################################################
         ...
         if(status==TRUE)
         {
           beta <- matrix(0,nrow=maxn-1,ncol=p)
           theta<-rep(0.5,maxn)
       ...
       #########################################################################################
		 ...
		 betal <- matrix(0,nrow=maxn-1,ncol=p)
		 betar <- matrix(0,nrow=maxn-1,ncol=p)
	 	 beta1 <- matrix(0,nrow=maxn-1,ncol=p)
		 thetal <- rep(0,maxn)
		 thetar <- rep(0,maxn)
		 theta1 <- rep(0,maxn)
       ...
       #########################################################################################
          ...
          foo <- .Fortran(F_tldbppdensity,
                 ...
                 thetal		=as.double(thetal),
                 thetar		=as.double(thetar),
       ...
       #########################################################################################
		 ...
		 state <- list(k=foo$kk,
					   g=foo$gp,
					   beta=matrix(foo$beta,nrow=maxn-1,ncol=p),
					   mub=foo$mub,
		 ...
		 randsave <- matrix(foo$randsave,nrow=nsave,ncol=(maxn-1)*p+maxn)
		 ...
		 {
			count<-count+1
			rnames[count]<-paste("theta",j,sep = "")
		 }
...
"print.tLDBPPdensity" <- function (x, digits = max(3, getOption("digits") - 3), ...)
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"summary.tLDBPPdensity"<-function(object, hpd=TRUE, ...)
{
    stde<-function(x)
    {
    	n<-length(x)
    ...
    hpdf<-function(x)
    {
         ...
         a<-.Fortran("hpd",n=as.integer(n),alpha=as.double(alpha),x=as.double(vec),
                     alow=as.double(alow),aupp=as.double(aupp),PACKAGE="SNSequate")
...
### g-prior
    ...
    class(ans) <- "summarytLDBPPdensity"
    return(ans)
}



"print.summarytLDBPPdensity"<-function (x, digits = max(3, getOption("digits") - 3), ...)
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
#HERE
    if (length(x$stickbreaking)) {
        cat("\nDP Stick-Breaking distribution:\n")
        print.default(format(x$stickbreaking, digits = digits), print.gap = 2,
            quote = FALSE)
    }
    else cat("No Stick-Breaking parameters\n")
...
"plot.tLDBPPdensity"<-function(x, hpd=TRUE, ask=TRUE, nfigr=2, nfigc=2, param=NULL, col="#bdfcc9", ...)
{
